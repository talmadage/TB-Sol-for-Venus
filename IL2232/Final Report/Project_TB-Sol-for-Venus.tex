...
The FPGA testbench implementation was evaluated in Quartus Prime using the Fitter and PowerPlay Power Analyzer reports on a Cyclone V 5CSEMA4U23C6 device. In terms of area, the design uses 328 ALMs and 187 registers, corresponding to about 2 percent of the available logic resources. Memory dominates the resource usage, with 1,048,576 block memory bits utilized and 128 RAM blocks instantiated, which corresponds to 38 percent of the available block memory bits and 47 percent of the available RAM blocks. The design uses 28 I/O pins and does not require any DSP blocks. Power estimation reports a total thermal power dissipation of 319.55 mW, dominated by core static power at 316.14 mW, while I/O thermal power contributes 3.41 mW. Overall, the implementation shows low logic cost and modest power consumption, with resource usage primarily driven by the on-chip memory allocated for instruction and data storage rather than by computational logic.

The same test was performed on the CISC processor. In terms of area, the design uses 3,345 ALMs and 1,430 registers, corresponding to about 21 percent of the available logic resources. The design uses 20 I/O pins, and it does not instantiate any on-chip block memory or DSP blocks, indicating that the implementation is primarily logic-based rather than memory-resource driven. Power estimation reports a total thermal power dissipation of 344.69 mW, dominated by core static power at 315.07 mW, while core dynamic thermal power contributes 18.82 mW and I/O thermal power contributes 10.81 mW. Overall, compared with the testbench platform, the processor consumes significantly more logic and exhibits a larger dynamic and I/O power component, which is consistent with active switching in the processor datapath and the inter-board communication signals.
